...
class MainView:
    ...
    def dashboard_top(self, window):
        ...
        # Setting up window for main application
        self.profile_frame(window)
        self.projects_frame(window)
        self.notes_frame(window)
    

    def profile_frame(self, window):
        
        # profile frame outline color
        profile_frame = ctk.CTkFrame(window, bg_color='transparent',
    ...
    def notes_frame(self, window):

        # notes label
        notes_label = ctk.CTkLabel(window, text='Notes',
        ...
        notes_frame = ctk.CTkFrame(window, bg_color='transparent',
                                    fg_color=self.MAIN_COLOR)
    ...
    def projects_frame(self, window):

        # project Label
        curr_project = ctk.CTkLabel(window, text='Projects',
                                     text_color=self.TEXT_COLOR, font=('',20,'bold'))
        curr_project.grid(row=0, column=3, columnspan=6, sticky='nw',pady=(3,0), padx=(5,0))

        # project name entry
        proj_name = ctk.CTkEntry(window, fg_color='#cccbc8', corner_radius=8, height=8)
        proj_name.grid(row=0, column=4, columnspan=4, sticky='new', pady=(5,5), padx=(3,3))

        # add project button
        add_project = ctk.CTkButton(window, text='Add Project', fg_color=self.MAIN_COLOR,
        ...
        projects_bg_frame = ctk.CTkFrame(window, bg_color='transparent',
                                          fg_color=self.MAIN_COLOR)
        projects_bg_frame.grid(row=0, column=3, rowspan=9, columnspan=6, 
                               sticky='news', pady=(32,2), padx=2)

        # project frame
        self.projects_frame = ctk.CTkScrollableFrame(window, bg_color='transparent',
                                            fg_color='transparent')
